                                        ...
                                        int8_t *output_data)
{
    ...
    const int32_t stride_h = conv_params->stride.w;
    const int32_t output_inc = output_dims->w * rhs_rows;
    const int32_t output_h = output_dims->h;
    const int32_t batch = input_dims->n;
    const int8_t **input_data_ref;
    const int32_t batch_offset = rhs_cols * input_dims->w * input_dims->h;

    for (int i_batch = 0; i_batch < batch; i_batch++)
    {
        input_data_ref = input_data;
        for (int i_output_h = 0; i_output_h < output_h; i_output_h++)
        {
            // Process one input row
            arm_cmsis_nn_status result = arm_nn_mat_mult_nt_t_s8(*input_data_ref,
                                                                 filter_data,
                                                                 bias_data,
                                                                 output_data,
...
arm_cmsis_nn_status arm_convolve_1x1_s8_contribs(const cmsis_nn_context *ctx,
                                        const cmsis_nn_conv_params *conv_params,
                                        const cmsis_nn_per_channel_quant_params *quant_params,
                                        const cmsis_nn_dims *input_dims,
                                        const int8_t * const *input_data,
                                        ...
                                        const int32_t *contribs)
{
    ...
    for (int i_batch = 0; i_batch < batch; i_batch++)
    {
        ...
        {
            // Process one input row
            arm_cmsis_nn_status result = arm_nn_mat_mult_nt_t_s8_contribs(*input_data_ref,
                                                                 filter_data,
                                                                 bias_data,
                                                                 output_data,
                                                                 quant_params->multiplier,
...
arm_cmsis_nn_status arm_convolve_1x1_s8_ori(const cmsis_nn_context *ctx,
                                        const cmsis_nn_conv_params *conv_params,
                                        const cmsis_nn_per_channel_quant_params *quant_params,
                                        const cmsis_nn_dims *input_dims,
