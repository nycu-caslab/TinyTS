...
class depthwiseInplace:
    ...
    def genFuncDefine(self):
        ...
        if self.fp_requantize:
            retString += """(int8_t * const *input, const uint16_t input_x, const uint16_t input_y,
                ...
                const uint16_t output_ch, int16_t *runtime_buf, int8_t pad_value, int pad_h, int pad_w)"""
        else:
            retString += """(int8_t * const *input, const uint16_t input_x, const uint16_t input_y,
                const uint16_t input_ch, const int8_t *kernel, const int32_t *bias, const int32_t *offsetBias,
    ...
    def _genKernelDefine(self):
        ...
        const int32_t *bias, const int32_t *offsetBias, const int8_t *ksrc, const float *scales,
...
"""
    ...
    for(i = 0; i < input_y + pad_h * 2; ++i) """
        )

        retString += """
        for (int p = 0; p < pad_h; ++p)
            *cols_8b++ = PAD8;"""
        retString += """
    }
...
"""
    ...
    for (c = 0; c < input_ch; c++){"""
        ...
        retString += self._assignInplaceOut(2, "output + c") # input
    ...
    def _genFixedLoadHWC2CWH(self):
        retString = (
            """
        cols_8b = (int8_t*)(cols_8b_start + pad_w """
            + """ * (input_y) + """
            + """ pad_h * pad_w * 2 """
            + """); //skip """
            + """ pad_w rows
    ...
    def _assignInplaceOut(self, pre_indent, out_str):
        return "    " * pre_indent + "int8_t *inplace_out = " + out_str + ";\n"
    ...
    def _genHandle1CH(self, pre_indent, out_offset_str):
        if self.fp_requantize:
            retString = (
                "    " * pre_indent
                + self._getKernelName()
                + "(output_y, output_x, bias++, offsetBias++, ksrc, scales++, inplace_out, output_offset,"
            ...
            retString = (
                "    " * pre_indent
                + self._getKernelName()
                + "(output_y, output_x, bias++, offsetBias++, ksrc, output_mult++, output_shift++, inplace_out,"
                + "output_offset,output_activation_min, output_activation_max,cols_8b_start, input_x_runtime_padded, "
                + out_offset_str
                + ", pad_h, pad_w);\n"
            )
        retString += "    " * pre_indent + "ksrc += " + str(self.kernel_h * self.kernel_w) + ";\n"
        # retString += "    " * pre_indent + "input++;" # hide
        return retString
    ...
    def _genHandle1CH_CWH(self, pre_indent, out_offset_str):
        if self.fp_requantize:
            retString = (
                "    " * pre_indent
                + self._getKernelName()
                + "(output_y, output_x, bias++, offsetBias++, ksrc, scales++, inplace_out,"
    ...
    def _genConvString(self):
        ...
        for i in range(self.kernel_h):
            if i > 0:
                retString += "            cols_8b += column_x + pad_w * 2;\n"
    ...
    def _genConvStringCWH(self):
        ...
        for i in range(self.kernel_w):
            if i > 0:
                retString += "            cols_8b += column_x + pad_h * 2 ;\n"
    ...
    def _genKernel(self):
        ...
        retString += "    int8_t *cols_8b_reset = cols_8b_iterptr;\n"
        retString += "    int i, j;\n"
    ...
    for (i = 0; i < output_y; i++) {
        ...
        """
        if self.stride != 1 or True: # TINY_TS: force override
            retString += "cols_8b_iterptr = cols_8b_reset + (column_x + pad_w * 2)*STRIDE;\n"
            retString += "        cols_8b_reset = cols_8b_iterptr;\n"
        else:
            retString += "cols_8b_iterptr += pad_w * 2;\n"
    ...
    for (j = 0; j < output_x; j++) {
        ...
        """
        if self.stride != 1:
            retString += "cols_8b_iterptr += pad_h * 2 - (column_x & 1);\n"
            retString += "        cols_8b_iterptr += (STRIDE - 1) * (column_x + pad_h * 2);\n"
        else:
            retString += "cols_8b_iterptr += pad_h * 2;\n"
